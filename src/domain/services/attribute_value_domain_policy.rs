//! Domain policy for AttributeValue aggregate
//!
...
/// Domain policy for AttributeValue — permits all operations (no business invariants).
///
...
/// enforcement, implement `backbone_core::ServiceLifecycle<AttributeValue>` on a custom
/// service. `PermitAllPolicy` is the safe default until then.
pub type AttributeValueDomainPolicy = PermitAllPolicy<AttributeValue>;
