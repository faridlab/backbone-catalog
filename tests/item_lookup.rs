...
//!
//! Tenancy: none, by design (ADR-0029) — the lookups run plain on the pool, undecorated.
//! Under a composed host's org request scope they ride the request-dedicated scoped
//! connection, so per-unit isolation is the decorator's, not the module's.
